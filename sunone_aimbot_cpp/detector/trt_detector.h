...
public:
    ...
    void processFrameGpu(cv::cuda::GpuMat& gpuFrame);
    void inferenceThread();

    // CUDA-D3D11 interop for zero-copy capture
    std::unique_ptr<CudaD3D11Interop> cudaInterop;
    bool initializeCudaInterop(ID3D11Device* d3dDevice);
...
private:
    ...
    void postProcess(
        ...
        const cv::Mat& sourceFrame = cv::Mat()
    );
