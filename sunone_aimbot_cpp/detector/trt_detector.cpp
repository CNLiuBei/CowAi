...
#include "../capture/cuda_d3d11_interop.h"
#include "nvinf.h"
...
{
    std::cout << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE5\x8A\xA0\xE8\xBD\xBD\xE6\xA8\xA1\xE5\x9E\x8B: " << modelFile << std::endl;
    std::cout.flush();
    ...
    {
        std::cerr << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE6\xA8\xA1\xE5\x9E\x8B\xE6\x96\x87\xE4\xBB\xB6\xE4\xB8\x8D\xE5\xAD\x98\xE5\x9C\xA8: " << modelFile << std::endl;
        return;
    }
    
    std::cout << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE6\xA8\xA1\xE5\x9E\x8B\xE6\x96\x87\xE4\xBB\xB6\xE5\xAD\x98\xE5\x9C\xA8, \xE6\x89\xA9\xE5\xB1\x95\xE5\x90\x8D: " << extension << std::endl;
    std::cout.flush();
    ...
    if (extension == ".engine")
    {
        ...
        auto fileSize = std::filesystem::file_size(modelFile);
        std::cout << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE5\xBC\x95\xE6\x93\x8E\xE6\x96\x87\xE4\xBB\xB6\xE5\xA4\xA7\xE5\xB0\x8F: " << fileSize << " \xE5\xAD\x97\xE8\x8A\x82" << std::endl;
        std::cout.flush();
        
        if (fileSize == 0)
        {
            std::cerr << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE5\xBC\x95\xE6\x93\x8E\xE6\x96\x87\xE4\xBB\xB6\xE4\xB8\xBA\xE7\xA9\xBA!" << std::endl;
            return;
        }
    }
    else if (extension == ".onnx")
    {
        ...
        {
            std::cout << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE4\xBB\x8E ONNX \xE6\xA8\xA1\xE5\x9E\x8B\xE6\x9E\x84\xE5\xBB\xBA\xE5\xBC\x95\xE6\x93\x8E..." << std::endl;
            std::cout.flush();
            ...
            if (builtEngine)
            {
                ...
                if (serializedEngine)
                {
                    ...
                    if (engineFile)
                    {
                        ...
                        std::cout << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE5\xBC\x95\xE6\x93\x8E\xE5\xB7\xB2\xE4\xBF\x9D\xE5\xAD\x98\xE5\x88\xB0: " << engineFilePath << std::endl;
                    }
                    else
                    {
                        std::cerr << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE6\x97\xA0\xE6\xB3\x95\xE5\x88\x9B\xE5\xBB\xBA\xE5\xBC\x95\xE6\x93\x8E\xE6\x96\x87\xE4\xBB\xB6: " << engineFilePath << std::endl;
                    }
            ...
            {
                std::cerr << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE4\xBB\x8E ONNX \xE6\x9E\x84\xE5\xBB\xBA\xE5\xBC\x95\xE6\x93\x8E\xE5\xA4\xB1\xE8\xB4\xA5" << std::endl;
                return;
            }
        }
        else
        {
            std::cout << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE4\xBD\xBF\xE7\x94\xA8\xE5\xB7\xB2\xE6\x9C\x89\xE5\xBC\x95\xE6\x93\x8E\xE6\x96\x87\xE4\xBB\xB6: " << engineFilePath << std::endl;
        }
    }
    else
    {
        std::cerr << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE4\xB8\x8D\xE6\x94\xAF\xE6\x8C\x81\xE7\x9A\x84\xE6\xA8\xA1\xE5\x9E\x8B\xE6\xA0\xBC\xE5\xBC\x8F: " << extension << std::endl;
        return;
    }

    std::cout << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE4\xBB\x8E\xE6\x96\x87\xE4\xBB\xB6\xE5\x8A\xA0\xE8\xBD\xBD\xE5\xBC\x95\xE6\x93\x8E..." << std::endl;
    std::cout.flush();
    ...
    if (engine)
    {
        std::cout << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE5\xBC\x95\xE6\x93\x8E\xE5\x8A\xA0\xE8\xBD\xBD\xE6\x88\x90\xE5\x8A\x9F" << std::endl;
    }
    else
    {
        std::cerr << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE5\xBC\x95\xE6\x93\x8E\xE5\x8A\xA0\xE8\xBD\xBD\xE5\xA4\xB1\xE8\xB4\xA5! \xE5\xBC\x95\xE6\x93\x8E\xE6\x96\x87\xE4\xBB\xB6\xE5\x8F\xAF\xE8\x83\xBD\xE4\xB8\x8E\xE5\xBD\x93\xE5\x89\x8D TensorRT \xE7\x89\x88\xE6\x9C\xAC\xE4\xB8\x8D\xE5\x85\xBC\xE5\xAE\xB9" << std::endl;
        std::cerr << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE8\xAF\xB7\xE5\xB0\x9D\xE8\xAF\x95\xE5\x88\xA0\xE9\x99\xA4 " << engineFilePath << " \xE5\xB9\xB6\xE9\x87\x8D\xE5\x90\xAF\xE7\xA8\x8B\xE5\xBA\x8F\xE4\xBB\xA5\xE4\xBB\x8E ONNX \xE9\x87\x8D\xE6\x96\xB0\xE6\x9E\x84\xE5\xBB\xBA" << std::endl;
    }
    std::cout.flush();
}
...
bool TrtDetector::initializeCudaInterop(ID3D11Device* d3dDevice)
{
    if (!d3dDevice)
    {
        std::cerr << "[TrtDetector] Invalid D3D11 device for CUDA interop" << std::endl;
        return false;
    }
    cudaInterop = std::make_unique<CudaD3D11Interop>();
    return cudaInterop->Initialize(d3dDevice);
}

void TrtDetector::processFrameGpu(cv::cuda::GpuMat& gpuFrame)
{
    if (config.backend == "DML") return;
    ...
    if (gpuFrame.empty() || !context) return;

    try
    {
        auto t0 = std::chrono::steady_clock::now();

        // Preprocess directly from GpuMat (skip upload)
        ...
        cv::cuda::resize(gpuBgr, gpuResizedBuffer, cv::Size(w, h), 0, 0, cv::INTER_LINEAR, cvStream);
        ...
        for (const auto& name : outputNames)
        {
            ...
            if (itPinned == pinnedOutputBuffers.end() || !itPinned->second)
                continue;
            cudaMemcpyAsync(itPinned->second, outputBindings[name], outputSizes[name],
        ...
        // Post-process
        // Download frame from GPU for dataset synchronization (only if dataset enabled)
        cv::Mat cpuFrame;
        ...
        for (const auto& name : outputNames)
        {
            ...
            if (dtype == nvinfer1::DataType::kHALF)
            {
                ...
                postProcess(outputDataFloat.data(), name, &lastNmsTime, cpuFrame);
            }
            else if (dtype == nvinfer1::DataType::kFLOAT)
            {
                postProcess(reinterpret_cast<const float*>(itPinned->second), name, &lastNmsTime, cpuFrame);
            }
        }
        ...
        lastInferenceTime = t_inf_end - t1;
        lastCopyTime = t_copy_end - t_inf_end;
        lastPostprocessTime = t_post_end - t_copy_end;
    }
    catch (const std::exception& e)
    {
        std::cerr << "[TrtDetector] GPU frame processing error: " << e.what() << std::endl;
    }
}
            ...
            try
            {
                ...
                for (const auto& name : outputNames)
                {
                    ...
                    if (dtype == nvinfer1::DataType::kHALF)
                    {
                        ...
                        postProcess(outputDataFloat.data(), name, &lastNmsTime, frame);
                    }
                    else if (dtype == nvinfer1::DataType::kFLOAT)
                    {
                        const float* floatPtr = reinterpret_cast<const float*>(itPinned->second);
                        postProcess(floatPtr, name, &lastNmsTime, frame);
                    }
                }
...
void TrtDetector::postProcess(const float* output, const std::string& outputName, std::chrono::duration<double, std::milli>* nmsTime, const cv::Mat& sourceFrame)
{
...

    {
        ...
        for (const auto& det : detections)
        {
            ...
            detectionBuffer.scores.push_back(det.confidence);
        }
        
        // Store the source frame for dataset synchronization (only if dataset enabled)
        // Release old frame first to prevent memory buildup
        if (!detectionBuffer.sourceFrame.empty())
            detectionBuffer.sourceFrame.release();
        if (config.dataset_enabled && !sourceFrame.empty())
            detectionBuffer.sourceFrame = sourceFrame.clone();
