...
void TrtDetector::loadEngine(const std::string& modelFile)
{
    ...
    if (extension == ".engine")
    {
        ...
        if (std::filesystem::file_size(modelFile) == 0)
        {
    ...
    else if (extension == ".onnx")
    {
        ...
        {
            std::cout << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE4\xBB\x8E ONNX \xE6\x9E\x84\xE5\xBB\xBA\xE5\xBC\x95\xE6\x93\x8E..." << std::endl;
            ...
            if (builtEngine)
            {
                ...
                if (serializedEngine)
                {
                    ...
                    {
                        std::cerr << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE6\x97\xA0\xE6\xB3\x95\xE5\x88\x9B\xE5\xBB\xBA\xE5\xBC\x95\xE6\x93\x8E\xE6\x96\x87\xE4\xBB\xB6" << std::endl;
                    }
    ...
    if (!engine)
    {
        std::cerr << "[\xE6\xA3\x80\xE6\xB5\x8B\xE5\x99\xA8] \xE5\xBC\x95\xE6\x93\x8E\xE5\x8A\xA0\xE8\xBD\xBD\xE5\xA4\xB1\xE8\xB4\xA5" << std::endl;
    }
}
            ...
            try
            {
                ...
                for (const auto& name : outputNames)
                {
                    ...
                    if (dtype == nvinfer1::DataType::kHALF)
                    {
                        ...
                        halfToFloatBuffer.resize(numElements);
                        for (size_t i = 0; i < numElements; ++i)
                            halfToFloatBuffer[i] = __half2float(halfPtr[i]);

                        postProcess(halfToFloatBuffer.data(), name, &lastNmsTime, frame);
                    }
