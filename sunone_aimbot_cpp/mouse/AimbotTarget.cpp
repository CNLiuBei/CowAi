...
// For both head and body: aim at vertical center for stability
// body_y_offset in config controls fine-tuning (0.5 = center)
static cv::Point calculateAimPoint(const cv::Rect& box, int classId, bool disableHeadshot)
{
    ...
    {
        // Head: always aim at vertical center for stability
        offsetY = box.height / 2;
    }
    else
    {
        // Body: use configurable offset (0.5 = center, <0.5 = higher, >0.5 = lower)
        // Default behavior: aim at center if body_y_offset is very small (legacy config)
        float effectiveOffset = config.body_y_offset;
        if (effectiveOffset < 0.3f)
        {
            // Legacy config with small offset - use center instead
            effectiveOffset = 0.5f;
        }
        offsetY = static_cast<int>(box.height * effectiveOffset);
        // Clamp to stay within box
        offsetY = std::max(1, std::min(offsetY, std::max(1, box.height - 1)));
    }
    ...
    const std::vector<std::pair<float, float>>* velocities)
{
    ...
    else if (lockedTrackId >= 0 && lockedIdx < 0)
    {
        ...
        else
        {
            ...
            // Return nullptr to stop aiming (don't switch to another person)
    ...
    return new AimbotTarget(finalX, finalY, finalW, finalH, finalClass, pivotX, pivotY, finalTrackId, associatedBodyId, finalVelX, finalVelY);
}
