...
private:
    ...
    double smoothed_pivot_x = 0.0;  // Smoothed pivot position for head-body switch
    double smoothed_pivot_y = 0.0;
    ...
    // Track our own mouse movement for velocity compensation
    double accumulated_mouse_dx = 0.0;
    double accumulated_mouse_dy = 0.0;
    std::mutex mouse_accum_mutex;
    ...
    std::pair<double, double> calc_movement(double target_x, double target_y, bool isShooting = false);
...
public:
    ...
    void moveMousePivot(double pivotX, double pivotY, double targetVelX = 0.0, double targetVelY = 0.0, bool isShooting = false);
    ...
    void pressMouseDirect();  // 直接点击，用于自动扳机
    void releaseMouse();
    void forceClick();  // 强制点击一次（不检查 mouse_pressed 状态），用于连点模式
    ...
    bool check_pivot_in_scope(double pivotX, double pivotY, 
        double targetW, double targetH, double tolerance);
    ...
    // Add external mouse movement to accumulated tracking (for recoil compensation)
    void addAccumulatedMovement(double dx, double dy)
    {
