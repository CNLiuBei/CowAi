...
{
    // The game's crosshair is 24 pixels ABOVE screen center
    // So we need to adjust our effective center to match the actual crosshair position
    // This way the mouse movement will align with where bullets actually go
    double effective_center_y = center_y - 24.0;  // Crosshair is 24px above screen center
...
bool MouseThread::check_target_in_scope(double target_x, double target_y, double target_w, double target_h, double reduction_factor)
{
    ...
    // The game's crosshair is 24 pixels ABOVE screen center
    double effective_center_y = center_y - 24.0;

    return (center_x >= x1 && center_x <= x2 && effective_center_y >= y1 && effective_center_y <= y2);
}
...
{
    // Use actual screen center for auto-shoot detection (no 24px offset)
    // The 24px offset is only for aiming movement, not for hit detection
    ...
    // Check if screen center is within the hit box
...
void MouseThread::moveMousePivot(double pivotX, double pivotY, double targetVelX, double targetVelY, bool isShooting)
{
    ...
    // The game's crosshair is 24 pixels ABOVE screen center
    // So we use effective_center_y for all offset calculations
    double effective_center_y = center_y - 24.0;
    ...
    double offset_y = effectivePivotY - effective_center_y;  // Use effective center for Y
    ...
    // Calculate base movement (without speed multiplier effects for prediction compensation)
    double offset_pred_x = pred_x - center_x;
    double offset_pred_y = pred_y - effective_center_y;  // Use effective center for Y
