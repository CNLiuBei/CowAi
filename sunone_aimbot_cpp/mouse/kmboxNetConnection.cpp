    ...
    : is_open_(false), ip_(ip), port_(port), uuid_(uuid), monitor_(false)
{
    ...
    if (!is_open_)
    {
        std::cerr << "[KmboxNet] 连接失败, ret=" << ret << std::endl;
        return;
    }

    std::cout << "[KmboxNet] 连接成功!" << std::endl;
...
{
    std::cout << "[KmboxNet] 正在启动监控, 端口 10000..." << std::endl;
    int ret = kmNet_monitor(10000);
    std::cout << "[KmboxNet] 监控已启动, ret=" << ret << std::endl;
