...
bool DuplicationAPIScreenCapture::createCroppedTexture()
{
    if (!d3dDevice) return false;
    ...
    HRESULT hr = d3dDevice->CreateTexture2D(&desc, nullptr, &croppedTexture);
    if (FAILED(hr))
    {
        std::cerr << "[DDA] CreateTexture2D(cropped) failed hr=" << std::hex << hr << std::endl;
        return false;
    }
    
    if (config.verbose)
    {
        std::cout << "[DDA] Created cropped texture " << regionWidth << "x" << regionHeight << std::endl;
    }
    return true;
}

GpuFrame DuplicationAPIScreenCapture::GetNextFrameGpu()
{
    GpuFrame result;
    ...
    {
        std::cerr << "[DDA] AcquireNextFrame (GPU) failed hr=0x"
            << std::hex << hr << std::endl;
        m_ddaManager->ReleaseFrame();
        return result;
    }
    ...
    // 释放原始帧纹理（但保留 DXGI 帧直到 ReleaseGpuFrame 调用）
    frameCtx.texture->Release();
    gpuFrameAcquired = true;

    result.texture = croppedTexture;
    result.width = regionWidth;
    result.height = regionHeight;
    result.valid = true;

    return result;
}

void DuplicationAPIScreenCapture::ReleaseGpuFrame()
{
    if (gpuFrameAcquired && m_ddaManager)
    {
        m_ddaManager->ReleaseFrame();
        gpuFrameAcquired = false;
    }
}
