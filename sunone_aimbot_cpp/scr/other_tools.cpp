...
std::vector<std::string> getModelFiles()
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator("models/"))
    {
            ...
            entry.is_regular_file() && entry.path().extension() == ".onnx")
        {
            ...
            modelsFiles.push_back(std::string(u8name.begin(), u8name.end()));
        }
    }
    return modelsFiles;
}
...
std::vector<std::string> getEngineFiles()
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator("models/"))
    {
        ...
        {
            auto u8name = entry.path().filename().u8string();
            engineFiles.push_back(std::string(u8name.begin(), u8name.end()));
        }
    }
    return engineFiles;
}
...
std::vector<std::string> getOnnxFiles()
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator("models/"))
    {
        ...
        {
            auto u8name = entry.path().filename().u8string();
            onnxFiles.push_back(std::string(u8name.begin(), u8name.end()));
        }
    }
    return onnxFiles;
}
    ...
    if (verbose)
    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < availableModels.size(); i++)
            {
                ...
                try {
                    fileSize = std::filesystem::file_size(utf8_to_path(modelPath));
                } catch (...) {}
