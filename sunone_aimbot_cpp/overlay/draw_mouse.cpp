...
void draw_mouse()
{
    ...
    if (config.auto_shoot)
    {
        ...
        if (SliderFloatWithKeys(u8"攻击间隔", &config.auto_shoot_interval, 50.0f, 500.0f, "%.0f ms", 10.0f))
        {
            config.saveConfig();
        }
    ...
    {
        ImGui::TextColored(ImVec4(255, 255, 255, 255), u8"这是标准鼠标输入方式，可能在大多数游戏中无效。请使用 GHUB 或 ARDUINO。");
        ImGui::TextColored(ImVec4(255, 0, 0, 255), u8"使用风险自负，此方法在某些游戏中会被检测。");
    }
