...
void draw_debug()
{
    ImGui::Checkbox(u8"显示调试窗口", &config.show_window);
    ...
    for (size_t i = 0; i < config.screenshot_button.size(); )
    {
        ...
        std::string combo_label = u8"截图按键 " + std::to_string(i);
        ...
        ImGui::SameLine();
        std::string remove_button_label = u8"移除##button_screenshot" + std::to_string(i);
    ...
    if (ImGui::Button(u8"添加按键##button_screenshot"))
    {
        config.screenshot_button.push_back("None");
        config.saveConfig();
    }

    ImGui::InputInt(u8"截图延迟", &config.screenshot_delay, 50, 500);
    ImGui::Checkbox(u8"详细控制台输出", &config.verbose);
