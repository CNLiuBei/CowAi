...
void draw_capture_settings()
{
    ...
    // GPU Zero-Copy option (only for duplication_api)
    if (config.capture_method == "duplication_api")
    {
        if (ImGui::Checkbox(u8"GPU 零拷贝优化", &config.gpu_zero_copy))
        {
            capture_method_changed.store(true);
            config.saveConfig();
        }
        ...
        {
            ImGui::SetTooltip(u8"启用后，D3D11 纹理将直接在 GPU 内存中传递，\n避免 GPU→CPU→GPU 的内存拷贝，可降低 1-3ms 延迟。");
        }
        
        // Display current capture path status
        ImGui::SameLine();
        if (zeroCopyActiveStatus.load())
        {
            ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), u8"[零拷贝激活]");
        }
        else if (config.gpu_zero_copy)
        {
            ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), u8"[等待初始化]");
        }
        else
        {
            ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), u8"[CPU 路径]");
        }
    }
